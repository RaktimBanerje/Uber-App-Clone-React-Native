  ...
  return (
    <View style={styles.container}>
        <View style={{alignItems: "center"}}>
          <Image 
            source={require(`../../../assets/cars/UberGo.png`)}
            style={{
              height: 150,
              width: 200
            }}
        ...
        <View style={{flex: 0.5, flexDirection: "row", justifyContent: "space-between"}}>
            <View> 
                <Text h2>{tripOption.name}</Text>
                <Text style={{fontSize: 20,fontWeight: "normal"}}>{format(currentDateTime, "p")} drop-off</Text>
                <Text style={{fontSize: 15, fontWeight: "normal"}}>{tripOption.description}</Text>
            </View>
            <View>
              <Text h2>{tripOption.price}</Text>
            </View>
        </View>
